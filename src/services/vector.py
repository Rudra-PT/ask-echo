...
def upsert_vectors(
    ...
    """Upsert vectors sharing identical supplementary metadata."""
    ...
    for batch_start in range(0, len(records), UPSERT_BATCH_SIZE):
        ...
        try:
            ...
            upserted_count: int = count if count is not None and count > 0 else len(batch)
...
def upsert_records(
    ...
    """Upsert vectors with per-chunk metadata into Pinecone."""
